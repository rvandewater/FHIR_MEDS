...
def robust_cast_time_column(pl_df):
    if "time" in pl_df.columns:
        # Remove trailing Z and timezone, then parse
        pl_df = pl_df.with_columns(
            pl.when(pl.col("time").is_not_null())
            .then(
                pl.col("time")
                .str.replace(r"Z$", "")  # Remove trailing Z
                .str.replace(r"([+-][0-9]{2}:[0-9]{2})$", "")  # Remove timezone
...
def write_single_shard(shard, required_cols, output_dir, shard_idx, verbose=False):
    try:
        ...
        table = DataSchema.validate(arrow_table)
        if verbose:
            print("Validated table:", table)
        pq.write_table(table, os.path.join(data_dir, f"{shard_idx}.parquet"))
        if verbose:
